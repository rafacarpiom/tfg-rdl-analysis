...
def _try_get_end_anchor_frame(rep: dict[str, Any]) -> int | None:
...
def build_minimal_comparative_context_from_bundle_and_reference(
    ...
    *,
    ...
    ideal_end_frame = _try_get_end_anchor_frame(ideal_rep)
    ...
    anchor_names = list(dict.fromkeys([*RDL_ANCHOR_NAMES, "end"]))
    for rep_order, user_rep_raw_index in enumerate(user_valid, start=1):
        ...
        user_end_frame = _try_get_end_anchor_frame(user_rep)
        anchors: dict[str, dict[str, Any]] = {}
        for anchor in anchor_names:
            uf = user_frames.get(anchor) if anchor != "end" else user_end_frame
            inf = ideal_frames.get(anchor) if anchor != "end" else ideal_end_frame
            pair_warnings: list[str] = []
            valid = True
            if anchor == "end":
                if not isinstance(uf, int):
                    valid = False
                    pair_warnings.append("USER_ANCHOR_INVALID:end")
                if not isinstance(inf, int):
                    valid = False
                    pair_warnings.append("IDEAL_ANCHOR_INVALID:end")
            else:
    ...
    return {
        ...
        "anchor_pairs": {"anchor_names": anchor_names, "paired_repetitions": paired_repetitions},
        "context_meta": {
...
def _run_from_context(ctx: dict, video_id: str) -> None:
    from src.visualization.rdl.detectors import export_lockout_debug

    result = detect_no_lockout_error(ctx)
    out_dir = OUTPUTS / "debug_detectors" / "rdl" / "repeticiones" / video_id / "lockout"
    export_lockout_debug(analysis_context=ctx, lockout_result=result, output_dir=out_dir)
    print(f"\nDebug lockout generado: {out_dir.as_posix()}")
...
def _run_bundle(bundle_dir: Path) -> None:
    for cache_name in ("08_analysis_context_anchor_cache.npz", "07_analysis_context_anchor_cache.npz"):
        ...
        if cache.is_file():
            ...
            print(f"Contexto lockout cargado desde cache: {cache_name}")
    ...
    if not missing:
        try:
            ...
            print("Contexto lockout reconstruido desde bundle + referencia ideal. No se ejecuta RTMPose.")
            _run_from_context(ctx, bundle_dir.name)
            return
    ...
    if missing:
        print("Faltan archivos para modo rápido lockout:")
        for p in missing:
            print(f"- {p.as_posix()}")
