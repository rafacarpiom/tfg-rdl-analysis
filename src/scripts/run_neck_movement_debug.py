...
def build_minimal_neck_movement_context_from_bundle_and_reference(
    bundle_dir: Path,
    reference_dir: Path,
    *,
    ...
    with np.load(str(reference_dir / "ideal_pose_sequence_normalized.npz"), allow_pickle=True) as npz:
        if "kps_xy_clean" in npz.files:
            ...
            ideal_kps_clean = np.asarray(npz["kps_xy_normalized"], dtype=np.float64)
            ideal_scores_clean = np.full((ideal_kps_clean.shape[0], 17), np.nan, dtype=np.float64)
    ...
    warnings: list[str] = []
    if used_normalized_fallback:
        warnings.append("IDEAL_CLEAN_MISSING_USING_NORMALIZED_FOR_NECK_MOVEMENT")
    ...
    for rep_order, user_rep_raw_index in enumerate(user_valid, start=1):
        ...
        for anchor in RDL_ANCHOR_NAMES:
            ...
            if valid and not (0 <= inf < ideal_kps_clean.shape[0]):
                valid = False
            ...
            anchors[anchor] = {
                ...
                "ideal_kps_clean": np.asarray(ideal_kps_clean[inf], dtype=np.float64) if valid else None,
                "user_kps_score_clean": np.asarray(user_scores_clean[uf], dtype=np.float64) if valid else None,
                "ideal_kps_score_clean": np.asarray(ideal_scores_clean[inf], dtype=np.float64) if valid else None,
    ...
    return {
        ...
        "reference": {
            ...
            "ideal_valid_rep_index": int(chosen_pos),
            "ideal_rep_raw_index": int(ideal_rep_raw_index),
            "pose_clean": {"kps_xy_clean": ideal_kps_clean, "kps_score_clean": ideal_scores_clean},
        },
        "anchor_pairs": {"anchor_names": list(RDL_ANCHOR_NAMES), "paired_repetitions": paired_repetitions},
        "context_meta": {
            "source": "minimal_neck_movement_context_from_bundle_and_reference",
            "video_id": bundle_dir.name,
...
def _run_from_context(ctx: dict, video_id: str) -> None:
    from src.visualization.rdl.detectors import export_neck_movement_debug

    result = detect_neck_movement_error(ctx)
    out_dir = OUTPUTS / "debug_detectors" / "rdl" / "repeticiones" / video_id / "neck_movement"
    export_neck_movement_debug(analysis_context=ctx, neck_movement_result=result, output_dir=out_dir)
    print(f"\nDebug neck_movement generado: {out_dir.as_posix()}")
...
def _run_bundle(bundle_dir: Path) -> None:
    for cache_name in ("08_analysis_context_anchor_cache.npz", "07_analysis_context_anchor_cache.npz"):
        ...
        if cache.is_file():
            ...
            print(f"Contexto neck_movement cargado desde cache: {cache_name}")
    ...
    if not missing:
        try:
            ctx = build_minimal_neck_movement_context_from_bundle_and_reference(bundle_dir, reference_dir, ideal_valid_rep_index=1)
            print("Contexto neck_movement reconstruido desde bundle + referencia ideal. No se ejecuta RTMPose.")
            _run_from_context(ctx, bundle_dir.name)
            return
    ...
    if missing:
        print("Faltan archivos para modo rápido neck_movement:")
        for p in missing:
            print(f"- {p.as_posix()}")
