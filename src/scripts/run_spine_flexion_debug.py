...
def _load_pose_clean_and_normalized(path: Path) -> tuple[np.ndarray, np.ndarray, np.ndarray | None, bool]:
    with np.load(str(path), allow_pickle=True) as npz:
        has_clean = "kps_xy_clean" in npz.files
        if "kps_xy_clean" in npz.files:
            clean = np.asarray(npz["kps_xy_clean"], dtype=np.float64)
        elif "kps_xy" in npz.files:
            clean = np.asarray(npz["kps_xy"], dtype=np.float64)
        elif "kps_xy_normalized" in npz.files:
            clean = np.asarray(npz["kps_xy_normalized"], dtype=np.float64)
        else:
            raise ValueError(f"{path.name} sin kps_xy_clean/kps_xy/kps_xy_normalized")
        if "kps_score_clean" in npz.files:
            scores = np.asarray(npz["kps_score_clean"], dtype=np.float64)
        elif "kps_score" in npz.files:
            scores = np.asarray(npz["kps_score"], dtype=np.float64)
        else:
            scores = np.full((clean.shape[0], 17), np.nan, dtype=np.float64)
        normalized = np.asarray(npz["kps_xy_normalized"], dtype=np.float64) if "kps_xy_normalized" in npz.files else None
    return clean, scores, normalized, has_clean


def build_minimal_spine_flexion_context_from_bundle_and_reference(bundle_dir: Path, reference_dir: Path, *, ideal_valid_rep_index: int = 1) -> dict:
    user_seg = _load_json(bundle_dir / "01_segmentation_result.json")
    user_clean, user_scores, user_norm, _ = _load_pose_clean_and_normalized(bundle_dir / "07_user_pose_clean.npz")
    ...
    ideal_clean, ideal_scores, ideal_norm, ideal_has_clean = _load_pose_clean_and_normalized(reference_dir / "ideal_pose_sequence_normalized.npz")
    used_ideal_norm_fallback = ideal_norm is not None and not ideal_has_clean
    ...
    for rep_order, user_rep_raw_index in enumerate(user_valid, start=1):
        ...
        for anchor in RDL_ANCHOR_NAMES:
            ...
            if valid and not (0 <= uf < user_clean.shape[0]):
                valid = False
                pair_warnings.append(f"USER_FRAME_OOB:{anchor}:{uf}")
            if valid and not (0 <= inf < ideal_clean.shape[0]):
                valid = False
            ...
            anchors[anchor] = {
                ...
                "user_kps_clean": np.asarray(user_clean[uf], dtype=np.float64) if valid else None,
                "ideal_kps_clean": np.asarray(ideal_clean[inf], dtype=np.float64) if valid else None,
                "user_kps_normalized": np.asarray(user_norm[uf], dtype=np.float64) if (valid and user_norm is not None) else None,
                "ideal_kps_normalized": np.asarray(ideal_norm[inf], dtype=np.float64) if (valid and ideal_norm is not None) else None,
                "user_kps_score_clean": np.asarray(user_scores[uf], dtype=np.float64) if valid else None,
                "ideal_kps_score_clean": np.asarray(ideal_scores[inf], dtype=np.float64) if valid else None,
    ...
    return {
        "user": {
            ...
            "pose_clean": {"kps_xy_clean": user_clean, "kps_score_clean": user_scores, "kps_xy_normalized": user_norm},
        ...
        "reference": {
            ...
            "ideal_valid_rep_index": int(chosen_pos),
            "ideal_rep_raw_index": int(ideal_rep_raw_index),
            "pose_clean": {"kps_xy_clean": ideal_clean, "kps_score_clean": ideal_scores, "kps_xy_normalized": ideal_norm},
        },
        "anchor_pairs": {"anchor_names": list(RDL_ANCHOR_NAMES), "paired_repetitions": paired_repetitions},
        "context_meta": {
            "source": "minimal_spine_flexion_context_from_bundle_and_reference",
            "video_id": bundle_dir.name,
...
def _run_from_context(ctx: dict, video_id: str) -> None:
    from src.visualization.rdl.detectors import export_spine_flexion_debug

    results: dict[str, Any] = {}
    results["hip_hinge"] = detect_hip_hinge(ctx)
    results["knee_dominant"] = detect_knee_dominant_error(ctx)
    results["neck_movement"] = detect_neck_movement_error(ctx)
    results["spine_flexion"] = detect_spine_flexion_error(analysis_context=ctx, detector_results=results)
    out_dir = OUTPUTS / "debug_detectors" / "rdl" / "repeticiones" / video_id / "spine_flexion"
    export_spine_flexion_debug(analysis_context=ctx, spine_flexion_result=results["spine_flexion"], output_dir=out_dir)
    print(f"\nDebug spine_flexion generado: {out_dir.as_posix()}")
    print(
        f"detected={results['spine_flexion'].get('detected', False)} "
        f"severity={results['spine_flexion'].get('severity', 'none')} "
        f"score={results['spine_flexion'].get('score', 0.0)}"
    )
...
def _run_bundle(bundle_dir: Path) -> None:
    for cache_name in ("08_analysis_context_anchor_cache.npz", "07_analysis_context_anchor_cache.npz"):
        ...
        if cache.is_file():
            ...
            print(f"Contexto spine_flexion cargado desde cache: {cache_name}")
    ...
    if not missing:
        try:
            ctx = build_minimal_spine_flexion_context_from_bundle_and_reference(bundle_dir, reference_dir, ideal_valid_rep_index=1)
            print("Contexto spine_flexion reconstruido desde bundle + referencia ideal. No se ejecuta RTMPose.")
            _run_from_context(ctx, bundle_dir.name)
            return
    ...
    if missing:
        print("Faltan archivos para modo rápido spine_flexion:")
        for p in missing:
            print(f"- {p.as_posix()}")
