...
def _load_pose_clean(path: Path) -> tuple[np.ndarray, np.ndarray]:
    with np.load(str(path), allow_pickle=True) as npz:
        if "kps_xy_clean" in npz.files:
            ...
            raise ValueError(f"{path.name} sin kps_xy_clean/kps_xy")
        if "kps_score_clean" in npz.files:
            ...
            scores = np.full((kps.shape[0], 17), np.nan, dtype=np.float64)
    return kps, scores


def build_minimal_rom_context_from_bundle_and_reference(
    bundle_dir: Path,
    reference_dir: Path,
    *,
    ...
    user_kps_clean, user_scores_clean = _load_pose_clean(bundle_dir / "07_user_pose_clean.npz")
    ...
    with np.load(str(reference_dir / "ideal_pose_sequence_normalized.npz"), allow_pickle=True) as npz:
        if "kps_xy_clean" in npz.files:
            ideal_kps_clean = np.asarray(npz["kps_xy_clean"], dtype=np.float64)
            ideal_scores_clean = np.asarray(npz["kps_score_clean"], dtype=np.float64) if "kps_score_clean" in npz.files else np.full((ideal_kps_clean.shape[0], 17), np.nan, dtype=np.float64)
            ideal_kps_normalized = np.asarray(npz["kps_xy_normalized"], dtype=np.float64) if "kps_xy_normalized" in npz.files else None
            fallback_norm = False
        elif "kps_xy_normalized" in npz.files:
            ideal_kps_clean = None
            ideal_scores_clean = None
            ideal_kps_normalized = np.asarray(npz["kps_xy_normalized"], dtype=np.float64)
            fallback_norm = True
        else:
            raise ValueError("ideal_pose_sequence_normalized.npz sin kps_xy_clean ni kps_xy_normalized")
    ...
    warnings: list[str] = []
    if fallback_norm:
        warnings.append("IDEAL_CLEAN_MISSING_USING_NORMALIZED_FOR_ROM")
    ...
    for rep_order, user_rep_raw_index in enumerate(user_valid, start=1):
        ...
        for anchor in RDL_ANCHOR_NAMES:
            ...
            if valid and not (0 <= uf < user_kps_clean.shape[0]):
                valid = False
                pair_warnings.append(f"USER_FRAME_OOB:{anchor}:{uf}")
            if valid:
                if ideal_kps_clean is not None and not (0 <= inf < ideal_kps_clean.shape[0]):
                    valid = False
                    pair_warnings.append(f"IDEAL_FRAME_OOB:{anchor}:{inf}")
                if ideal_kps_clean is None and (ideal_kps_normalized is None or not (0 <= inf < ideal_kps_normalized.shape[0])):
                    valid = False
                    pair_warnings.append(f"IDEAL_FRAME_OOB:{anchor}:{inf}")
            anchors[anchor] = {
                "anchor": anchor,
                ...
                "ideal_kps_clean": np.asarray(ideal_kps_clean[inf], dtype=np.float64) if (valid and ideal_kps_clean is not None) else None,
                "ideal_kps_normalized": np.asarray(ideal_kps_normalized[inf], dtype=np.float64) if (valid and ideal_kps_normalized is not None) else None,
    ...
    reference_pose_clean = {"kps_xy_clean": ideal_kps_clean, "kps_score_clean": ideal_scores_clean} if ideal_kps_clean is not None else {}
    out: dict[str, Any] = {
        "user": {
            "video_id": bundle_dir.name,
            "pose_clean": {"kps_xy_clean": user_kps_clean, "kps_score_clean": user_scores_clean},
        ...
        "context_meta": {
            "source": "minimal_rom_context_from_bundle_and_reference",
            "video_id": bundle_dir.name,
    ...
    if reference_pose_clean:
        out["reference"]["pose_clean"] = reference_pose_clean
    return out


def _run_from_context(ctx: dict, video_id: str) -> None:
    from src.visualization.rdl.detectors import export_rom_debug

    result = detect_short_rom_error(ctx)
    out_dir = OUTPUTS / "debug_detectors" / "rdl" / "repeticiones" / video_id / "rom"
    export_rom_debug(analysis_context=ctx, rom_result=result, output_dir=out_dir)
    print(f"\nDebug rom generado: {out_dir.as_posix()}")
...
def _run_bundle(bundle_dir: Path) -> None:
    for cache_name in ("08_analysis_context_anchor_cache.npz", "07_analysis_context_anchor_cache.npz"):
        ...
        if cache.is_file():
            ...
            print(f"Contexto rom cargado desde cache: {cache_name}")
    ...
    if not missing:
        try:
            ctx = build_minimal_rom_context_from_bundle_and_reference(bundle_dir, reference_dir, ideal_valid_rep_index=1)
            print("Contexto rom reconstruido desde bundle + referencia ideal. No se ejecuta RTMPose.")
            _run_from_context(ctx, bundle_dir.name)
            return
    ...
    if missing:
        print("Faltan archivos para modo rápido rom:")
        for p in missing:
            print(f"- {p.as_posix()}")
